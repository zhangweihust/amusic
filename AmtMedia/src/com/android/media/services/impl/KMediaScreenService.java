...
public class KMediaScreenService implements IKMediaScreenService {
	...
	public boolean show(Class<? extends IScreen> screen, boolean addToBack, ScreenArgs args, int visibility) {
		ScreenKMediaRoot kMediaRoot = ServiceManager.getKMediaRoot();
		if (args == null) {
			args = new ScreenArgs();
		}
		Intent intent = new Intent(kMediaRoot, screen);
		intent.putExtra("args", args);
		intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
		String screen_id = screen.getCanonicalName();
		Window window = kMediaRoot.getLocalActivityManager().startActivity(screen_id, intent);
		if (window != null) {
			if (addToBack) {
			...
			LinearLayout root = (LinearLayout) kMediaRoot.findViewById(R.id.screen_kmedia_root_view);
	...
	public boolean show(String screen_id, boolean addToBack, ScreenArgs args) {
		final IScreen screen = (IScreen) ServiceManager.getKMediaRoot().getLocalActivityManager().getActivity(screen_id);
