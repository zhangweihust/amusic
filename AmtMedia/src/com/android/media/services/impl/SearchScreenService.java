...
public class SearchScreenService implements ISearchScreenService {
	...
	public boolean show(Class<? extends IScreen> screen, boolean addToBack, ScreenArgs args, int visibility) {
		ScreenSearchRoot searchRoot = ServiceManager.getSearchRoot();
		if (args == null) {
			args = new ScreenArgs();
		}
		Intent intent = new Intent(searchRoot, screen);
		intent.putExtra("args", args);
		intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
		String screen_id = screen.getCanonicalName();
		Window window = searchRoot.getLocalActivityManager().startActivity(screen_id, intent);
		if (window != null) {
			if (addToBack) {
			...
			LinearLayout root = (LinearLayout) searchRoot.findViewById(R.id.screen_search_root_view);
	...
	public boolean show(String screen_id, boolean addToBack, ScreenArgs args) {
		final IScreen screen = (IScreen) ServiceManager.getSearchRoot().getLocalActivityManager().getActivity(screen_id);
