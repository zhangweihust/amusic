...
public class DeviceInfoThread extends Thread {
	...
	public DeviceInfoThread(int times, IUserInfoService countService) {
	...
	private boolean uploadCountToServer() {
		...
		if(networkService.acquire(false)){
			try {
				...
				params.add(new BasicNameValuePair(InfoName.SCREEN_DENSITYDPI.toString(), DeviceInformation.getInformation(InfoName.SCREEN_DENSITYDPI)));
